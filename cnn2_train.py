...
def accuracy(X, Y, parameters):
    predictions, probabilities = predict_cnn2(X, parameters)
...
def evaluate_cnn2(X_test, Y_test, parameters):
    AL, _ = cnn2_forward(
        X_test,
        parameters,
        conv_keep_prob=1.0,
...
if __name__ == "__main__":
    X_train = np.random.randn(8, 16, 16, 1)
    Y_train = np.array([[1], [0], [1], [0], [1], [0], [1], [0]])

    X_val = np.random.randn(4, 16, 16, 1)
    Y_val = np.array([[1], [0], [1], [0]])

    parameters, history = train_cnn2(
        X_train,
        Y_train,
        X_val=X_val,
        Y_val=Y_val,
    ...
    test_cost, test_acc, preds, probs = evaluate_cnn2(
        X_val,
        Y_val,
        parameters
    )
