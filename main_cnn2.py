...
from cnn2_train import create_mini_batches, learning_rate_decay


MODEL_NAME = "cnn2"
MODEL_TITLE = "CNN2 Inception"
RESULTS_DIR = "results/cnn2"
PLOTS_DIR = "results/cnn2/plots"
METRICS_DIR = "results/cnn2/metrics"
...
def evaluate_model(X, Y, parameters):
    ...
    cost = compute_cost(AL, Y, parameters=None, lambd=0.0)
...
def plot_cost_curve(history, fold_number):
    ...
    save_path = f"{PLOTS_DIR}/cnn2_fold_{fold_number}_cost_curve.png"

    if plt is None:
        save_fallback_line_plot(
            [history["train_cost"], history["val_cost"]],
            ["Train cost", "Validation cost"],
            f"{MODEL_TITLE} Cost Curve - Fold {fold_number}",
            "Epoch",
            "Cost",
            save_path
        )
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Cost")
    plt.title(f"{MODEL_TITLE} Cost Curve - Fold {fold_number}")
    plt.legend()
    plt.grid(True)
    save_matplotlib_figure(save_path)
    finish_plot()


def plot_accuracy_curve(history, fold_number):
    epochs = np.arange(1, len(history["train_accuracy"]) + 1)
    save_path = f"{PLOTS_DIR}/cnn2_fold_{fold_number}_accuracy_curve.png"

    if plt is None:
        save_fallback_line_plot(
            [history["train_accuracy"], history["val_accuracy"]],
            ["Train accuracy", "Validation accuracy"],
            f"{MODEL_TITLE} Accuracy Curve - Fold {fold_number}",
            "Epoch",
            "Accuracy",
            save_path
        )
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title(f"{MODEL_TITLE} Accuracy Curve - Fold {fold_number}")
    plt.legend()
    plt.grid(True)
    save_matplotlib_figure(save_path)
    finish_plot()
...
def plot_confusion_matrix(matrix, fold_number):
    save_path = f"{PLOTS_DIR}/cnn2_fold_{fold_number}_confusion_matrix.png"

    if plt is None:
        save_fallback_confusion_matrix(
            matrix,
            f"{MODEL_TITLE} Confusion Matrix - Fold {fold_number}",
            save_path
        )
        return
    ...
    plt.imshow(matrix)
    plt.title(f"{MODEL_TITLE} Confusion Matrix - Fold {fold_number}")
...
def plot_total_cost_curve(all_histories):
    ...
    save_path = f"{PLOTS_DIR}/cnn2_total_cost_curve.png"
    output_path = "Output/CNN2Cost.jpg"

    if plt is None:
        save_fallback_line_plot(
            [np.mean(train_costs, axis=0), np.mean(val_costs, axis=0)],
            ["Mean train cost", "Mean validation cost"],
            f"{MODEL_TITLE} Total Cost Curve Across Folds",
            "Epoch",
            "Cost",
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Cost")
    plt.title(f"{MODEL_TITLE} Total Cost Curve Across Folds")
    plt.legend()
    plt.grid(True)
...
def plot_total_accuracy_curve(all_histories):
    ...
    save_path = f"{PLOTS_DIR}/cnn2_total_accuracy_curve.png"
    output_path = "Output/CNN2Accuracy.jpg"
    ...
    if plt is None:
        save_fallback_line_plot(
            ...
            f"{MODEL_TITLE} Total Accuracy Curve Across Folds",
            "Epoch",
            "Accuracy",
            [save_path, output_path]
        )
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title(f"{MODEL_TITLE} Total Accuracy Curve Across Folds")
    plt.legend()
    plt.grid(True)
    save_matplotlib_figure([save_path, output_path])
    finish_plot()
...
def plot_total_confusion_matrix(matrix):
    save_path = f"{PLOTS_DIR}/cnn2_total_confusion_matrix.png"
    output_path = "Output/CNN2ConfMatrix.jpg"

    if plt is None:
        save_fallback_confusion_matrix(
            matrix,
            f"{MODEL_TITLE} Total Confusion Matrix Across Folds",
            [save_path, output_path]
        )
        return
    ...
    plt.figure(figsize=(5, 4))
    plt.imshow(matrix)
    plt.title(f"{MODEL_TITLE} Total Confusion Matrix Across Folds")
    plt.xlabel("Predicted label")
    plt.ylabel("True label")
    plt.xticks([0, 1], ["No tumor", "Tumor"])
    plt.yticks([0, 1], ["No tumor", "Tumor"])
...
def run_cnn2_experiment(
    ...
    print()
    print("=" * 70)
    print("CNN2 Inception Experiment Started")
    print("=" * 70)
    ...
    print("Block 1 filters:", block1_filters)
    print("Block 2 filters:", block2_filters)
    ...
    print()
    print("=" * 70)
    print("CNN2 K-Fold Final Summary")
    print("=" * 70)
...
def save_updated_parameters(
    ...
    save_path="results/cnn2/metrics/updated_parameters_2.json"
):
    output = {
        "model": MODEL_NAME,
        "architecture": (
            "InceptionBlock1-Concatenate-Dropout-"
            "InceptionBlock2-Concatenate-Dropout-"
            "Flatten-Dense64-ReLU-Dropout-Dense1-Sigmoid"
        ),
...
def save_experiment_summary(
    ...
    save_path = "results/cnn2/metrics/cnn2_kfold_summary.json"
